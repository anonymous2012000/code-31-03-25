/*
// This code:
(1) Constructs number_of_poly unforgeable polynomals
...
// Function to perform DivRem and check divisibility
bool verify_unforgeable_polynomial(const ZZ_pX& numerator, const ZZ_pX& denominator,const ZZ_pX& gamma) {
  ZZ_pX q, r;
  DivRem(q, r, numerator - gamma, denominator);
  return (r == 0);
}
...
int main() {
    ...
    long degree_pi = 100000000; // Degree of the polynomial \pi(x)
    ...
    for(int j = 0; j< number_of_experiments; j++){
      ...
      vector<ZZ_p> zeta_coeffs = generateRandomCoeffs(degree_zeta, field_size);
      // Convert the vector to ZZ_pX polynomials
      ZZ_pX zeta = vectorToPolynomial(zeta_coeffs);
      ...
      cout << "\n Is divisible (Y = 1, N = 0): " << is_divisible << endl;
